...
def button_click(number):
    #e.delete(0, END) means deleting everything in the text 
    current = e.get()
    e.delete(0, END) #deleting what is in there first
    e.insert(0, str(current) + str(number)) #make sure they are strings as u insert back into the text
def button_clear():
    e.delete(0, END)
...
button_clear = Button(root , text="Clear" , padx=79,pady=40, command =  button_clear)
